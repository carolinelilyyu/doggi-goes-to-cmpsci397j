...
// lib/game/entities/movingsky.js
ig.baked=true;ig.module('game.entities.movingsky').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){MovingSky=ig.Entity.extend({size:{x:800,y:600},animSheet:new ig.AnimationSheet('media/sky.png',800,600),init:function(x,y,settings){this.addAnim('background',1,[0],false);this.maxVel.x=20;this.maxVel.y=0;this.vel.x=-20;this.parent(x,y,settings);},update:function(){this.parent();if(this.pos.x<-800){this.pos.x=800;}
if(ig.game.state==ig.game.GAME_OVER){this.kill();}}});});

// lib/game/entities/movinggrass.js
ig.baked=true;ig.module('game.entities.movinggrass').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){MovingGrass=ig.Entity.extend({size:{x:800,y:600},animSheet:new ig.AnimationSheet('media/grass.png',800,600),init:function(x,y,settings){this.addAnim('background',1,[0],false);this.maxVel.x=20;this.maxVel.y=0;this.vel.x=-20;this.parent(x,y,settings);},update:function(){this.parent();if(this.pos.x<-800){this.pos.x=800;}
if(ig.game.state==ig.game.GAME_OVER){this.kill();}}});});

// lib/game/entities/movinghills.js
ig.baked=true;ig.module('game.entities.movinghills').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){MovingHills=ig.Entity.extend({size:{x:800,y:600},animSheet:new ig.AnimationSheet('media/hills.png',800,600),init:function(x,y,settings){this.addAnim('background',1,[0],false);this.maxVel.x=30;this.maxVel.y=0;this.vel.x=-30;this.parent(x,y,settings);},update:function(){this.parent();if(this.pos.x<-800){this.pos.x=800;}
if(ig.game.state==ig.game.GAME_OVER){this.kill();}}});});

// lib/game/entities/movingdubois.js
ig.baked=true;ig.module('game.entities.movingdubois').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){MovingDubois=ig.Entity.extend({size:{x:130,y:200},animSheet:new ig.AnimationSheet('media/dubois.png',130,200),init:function(x,y,settings){this.addAnim('background',1,[0],false);this.maxVel.x=50;this.maxVel.y=0;this.vel.x=-50;this.parent(x,y,settings);},update:function(){this.parent();if(this.pos.x<-ig.system.width){this.pos.x=ig.system.width;}
if(ig.game.state==ig.game.GAME_OVER){this.kill();}}});});

// lib/game/entities/toast.js
ig.baked=true;ig.module('game.entities.toast').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){Toast=ig.Entity.extend({type:ig.Entity.TYPE.A,size:{x:30,y:30},offset:{x:20,y:20},maxVel:{x:600,y:0},gravityFactor:0,animSheet:new ig.AnimationSheet('media/toast.png',50,50),init:function(x,y,settings){this.vel.x=-600;this.maxVel.x=600;this.maxVel.y=0;this.addAnim('idle',1,[0],false);this.parent(x,y,settings);},update:function(){this.parent();if(this.pos.x<(0-50)){this.kill();}
if(ig.game.state==ig.game.GAME_OVER){this.kill();}},reset:function(x,y,settings){this.parent(x,y,settings);this.vel.x=-600;},});});

// lib/game/entities/truck.js
ig.baked=true;ig.module('game.entities.truck').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){Truck=ig.Entity.extend({type:ig.Entity.TYPE.A,size:{x:300,y:140},maxVel:{x:1000,y:600},gravityFactor:0,hittruck:false,animSheet:new ig.AnimationSheet('media/truck.png',300,190),init:function(x,y,settings){this.vel.x=-600;this.maxVel.x=600;this.maxVel.y=0;this.addAnim('idle',1,[0],false);this.parent(x,y,settings);},update:function(){this.parent();if(this.hittruck&&this.pos.x<ig.system.width){this.currentAnim.angle+=(this.vel.x>0)?0.1:-0.1;}
if(this.pos.x<0-300){this.kill();}if(ig.game.state==ig.game.GAME_OVER){this.kill();}},reset:function(x,y,settings){this.parent(x,y,settings);this.vel.x=-600;this.hittruck=false;this.currentAnim.angle=0;},});});

// lib/game/entities/powerup.js
ig.baked=true;ig.module('game.entities.powerup').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){PowerUp=ig.Entity.extend({type:ig.Entity.TYPE.A,size:{x:50,y:60},maxVel:{x:600,y:0},gravityFactor:0,animSheet:new ig.AnimationSheet('media/powerup.png',50,60),init:function(x,y,settings){this.vel.x=-600;this.maxVel.x=600;this.maxVel.y=0;this.addAnim('idle',1,[0],false);this.parent(x,y,settings);},update:function(){this.parent();if(this.pos.x<0-50){this.kill();}if(ig.game.state==ig.game.GAME_OVER){this.kill();}},reset:function(x,y,settings){this.parent(x,y,settings);this.vel.x=-600;},});});

// lib/game/entities/bird.js
ig.baked=true;ig.module('game.entities.bird').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){Bird=ig.Entity.extend({type:ig.Entity.TYPE.A,size:{x:110,y:50},offset:{x:20,y:30},maxVel:{x:900,y:300},gravityFactor:0,hitbird:false,animSheet:new ig.AnimationSheet('media/bird.png',130,80),init:function(x,y,settings){this.vel.x=-900;this.maxVel.x=900;this.maxVel.y=0;this.addAnim('idle',1,[0],false);this.parent(x,y,settings);},update:function(){if(this.hitbird&&this.pos.x<ig.system.width){this.currentAnim.angle+=(this.vel.x>0)?0.1:-0.1;}
this.parent();if(this.pos.x<0-130){console.log("kill bird")
this.kill();}if(ig.game.state==ig.game.GAME_OVER){this.kill();}},reset:function(x,y,settings){this.parent(x,y,settings);this.vel.x=-900;this.hitbird=false;this.currentAnim.angle=0;},});});

// lib/game/entities/ground1.js
ig.baked=true;ig.module('game.entities.ground1').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){Ground1=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/ground1.png',548,80),size:{x:548,y:80},offset:{x:0,y:20},maxVel:{x:600,y:0},type:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.FIXED,gravityFactor:0,isItAlive:false,init:function(x,y,settings){this.addAnim('idle',1,[0],false);this.maxVel.x=600;this.maxVel.y=0;this.vel.x=-600;this.parent(x,y,settings);},update:function(){if(this.pos.x+this.size.x<0){this.kill();this.isItAlive=false;}if(ig.game.state==ig.game.GAME_OVER){this.kill();this.isItAlive=false;}
this.parent();},reset:function(x,y,settings){this.parent(x,y,settings);this.vel.x=-600;},toString:function(){return"ground1";},});});

// lib/game/entities/ground2.js
ig.baked=true;ig.module('game.entities.ground2').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){Ground2=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/ground2.png',548,80),offset:{x:0,y:20},size:{x:548,y:80},maxVel:{x:600,y:0},type:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.FIXED,gravityFactor:0,init:function(x,y,settings){this.addAnim('idle',1,[0],false);this.maxVel.x=600;this.maxVel.y=0;this.vel.x=-600;this.parent(x,y,settings);},update:function(){if(this.pos.x+this.size.x<0){this.kill();}if(ig.game.state==ig.game.GAME_OVER){this.kill();}
this.parent();},reset:function(x,y,settings){this.parent(x,y,settings);this.vel.x=-600;},toString:function(){return"ground2";},});});

// lib/game/entities/ground3.js
ig.baked=true;ig.module('game.entities.ground3').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){Ground3=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/ground3.png',548,80),size:{x:548,y:80},offset:{x:0,y:20},maxVel:{x:600,y:0},type:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.FIXED,gravityFactor:0,init:function(x,y,settings){this.addAnim('idle',1,[0],false);this.maxVel.x=600;this.maxVel.y=0;this.vel.x=-600;this.parent(x,y,settings);},update:function(){if(this.pos.x+this.size.x<0){this.kill();}if(ig.game.state==ig.game.GAME_OVER){this.kill();}
this.parent();},reset:function(x,y,settings){this.parent(x,y,settings);this.vel.x=-600;},toString:function(){return"ground3";},});});

// lib/game/entities/ground4.js
ig.baked=true;ig.module('game.entities.ground4').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){Ground4=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/ground4.png',1644,80),size:{x:1644,y:80},offset:{x:0,y:20},maxVel:{x:600,y:0},type:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.FIXED,gravityFactor:0,init:function(x,y,settings){this.addAnim('idle',1,[0],false);this.maxVel.x=600;this.maxVel.y=0;this.vel.x=-600;this.parent(x,y,settings);},update:function(){if(this.pos.x+this.size.x<0){this.kill();}if(ig.game.state==ig.game.GAME_OVER){this.kill();}
this.parent();},reset:function(x,y,settings){this.parent(x,y,settings);this.vel.x=-600;},toString:function(){return"ground4";},});});

// lib/game/entities/player.js
ig.baked=true;ig.module('game.entities.player').requires('impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound').defines(function(){Player=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/spritesheet.png',(1600/10),200),size:{x:(1600/10),y:200},ground:false,jumpVel:-600,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.ACTIVE,damping:1.2,jumpCount:0,powerupmode:false,crouching:false,stun:false,hitgroundnormal:false,poweruptimer:new ig.Timer(),playerhitbird:false,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('moving',0.1,[0,1],false);this.addAnim('jumping',1,[2],false);this.addAnim('sliding',0.1,[3,4],false);this.addAnim('powerupmoving',0.1,[5,6,7,8,9],false);this.maxVel.x=(600);},handleMovementTrace:function(res){this.parent(res);if(res.collision.x){console.log("hitsides")
this.vel.y=0;this.vel.x=0;}},checkKill:function(){this.parent();if(this.pos.y>ig.system.height|this.pos.x<-100){this.kill();ig.game.state=ig.game.GAME_OVER;}},check:function(other){if(other instanceof Ground1){this.touchedPlatform=true;this.ground=true;}else if(other instanceof Ground2){this.touchedPlatform=true;this.ground=true;}else if(other instanceof Ground3){this.ground=true;}else if(other instanceof Ground4){this.touchedPlatform=true;this.ground=true;}
if(other instanceof Toast){other.kill();ig.game.score+=1;}
if(other instanceof PowerUp){this.powerUp();other.kill();}
if(!this.powerupmode&&other instanceof Truck){this.stun=true;this.vel.x=0;this.pos.x-=10;}else if(this.powerupmode&&other instanceof Truck){other.vel.y=-300;other.maxVel.y=300;other.vel.x=1000;other.maxVel.x=1000;other.hittruck=true;}
if(!this.powerupmode&&other instanceof Bird){this.playerhitbird=true;this.vel.x=-600;this.vel.y=-600;}else if(this.powerupmode&&other instanceof Bird){other.vel.y=300;other.maxVel.y=300;other.vel.x=1000;other.maxVel.x=1000;other.hitbird=true;}},powerUp:function(){this.offset.y=0;this.size.y=200;this.powerupmode=true;this.poweruptimer.set(5);},update:function(){if(this.playerhitbird&&this.pos.x+this.size.x>=0){this.pos.x-=10;this.pos.y-=10;ig.game.player.currentAnim.angle+=(ig.game.player.vel.x>0)?0.1:-0.1;}
if(this.powerupmode&&this.poweruptimer.delta()>=0){this.powerupmode=false;this.size.y=120;this.offset.y=80;this.pos.y+=80;}
this.checkStationary();this.updateAnims();this.checkInputs();this.checkKill();this.parent();},checkStationary:function(){if(this.ground){this.vel.x=300;this.jumpCount=0;}else if(this.ground==false){this.vel.x=0;}if(this.vel.y>0){this.ground=false;}if(!this.ground&&!this.stun&&!this.playerhitbird){this.pos.x=100}},updateAnims:function(){if(this.powerupmode){this.currentAnim=this.anims.powerupmoving;}
else{if(ig.input.state('up')){this.currentAnim=this.anims.jumping;}else if(!this.ground){this.currentAnim=this.anims.jumping;}else if(ig.input.state('down')){this.currentAnim=this.anims.sliding;}else{this.currentAnim=this.anims.moving;}}},checkInputs:function(){if(ig.input.state('up')&&(this.jumpCount==0)){this.vel.y=this.jumpVel;this.ground=false;this.jumpCount=1;}
if(ig.input.state('space')&&(this.touchedPlatform==true)&&(this.jumpCount==1)){this.vel.y=this.jumpVel/2;this.jumpCount=0;this.touchedPlatform=false;}
if(!ig.input.state('up')&&(this.ground==false)&&(this.vel.y<0)){this.vel.y=(this.vel.y)/this.damping;}if(ig.input.state('down')&&!this.crouching&&this.ground&&!this.powerupmode){this.offset.y=160;this.size.y=40;this.pos.y+=80;this.crouching=true;this.hitgroundnormal=false;}
else if(!ig.input.state('down')&&!this.hitgroundnormal&&!this.powerupmode){this.crouching=false;if(this.size.y==40){this.pos.y-=80}else{this.pos.y+=80;}
this.size.y=120;this.offset.x=30;this.size.x=100;this.offset.y=80;this.hitgroundnormal=true;}},});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','impact.image','impact.entity','impact.entity-pool','impact.sound','impact.debug.debug','game.entities.movingsky','game.entities.movinggrass','game.entities.movinghills','game.entities.movingdubois','game.entities.toast','game.entities.truck','game.entities.powerup','game.entities.bird','game.entities.ground1','game.entities.ground2','game.entities.ground3','game.entities.ground4','game.entities.player').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/score.font.png'),highscore:0,score:0,title:new ig.Image('media/title.png'),background:new ig.Image('media/background.png'),transparentbox:new ig.Image('media/transparentbox.png'),instructionsbackground:new ig.Image('media/instructions.png'),gameoverbackground:new ig.Image('media/gameover.png'),gameoverwords:new ig.Image('media/gameoverwords.png'),music:new ig.Sound('media/music.*'),doggi:new ig.Image('media/titledoggi.png'),clearColor:null,gravity:100,state:0,MENU:0,INSTRUCTIONS:1,CREDITS:2,GAME:3,GAME_OVER:4,currentPlatform:null,yPos:0,xSpace:0,initialPlatform:null,player:null,init:function(){if(this.state==this.MENU){this.music.play();}
ig.EntityPool.enableFor(Ground1);ig.EntityPool.enableFor(Ground2);ig.EntityPool.enableFor(Ground3);ig.EntityPool.enableFor(Ground4);ig.EntityPool.enableFor(Toast);ig.EntityPool.enableFor(Truck);ig.EntityPool.enableFor(PowerUp);ig.EntityPool.enableFor(Bird);ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.SPACE,'space');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.BACKSPACE,'back');},update:function(){switch(this.state){case this.MENU:console.log("in menu");if(ig.input.state('space')){this.startGame();}if(ig.input.state('up')){this.state=this.INSTRUCTIONS;}if(ig.input.state('down')){this.state=this.CREDITS}
break;case this.INSTRUCTIONS:if(ig.input.state('space')){this.startGame();}
break;case this.CREDITS:if(ig.input.state('space')){this.startGame();}
break;case this.GAME:console.log(this.initialPlatform.isItAlive);if(this.currentPlatform!=null&&this.currentPlatform.pos.x<=ig.system.width){this.yPos=(Math.random()*this.currentPlatform.size.y+80)+ig.system.height-(this.currentPlatform.size.y*2)-60;if(this.currentPlatform.toString()!=("ground4")){this.xSpace=(Math.random())*(ig.system.width/2)+(ig.system.width*2);}else{this.xSpace=(Math.random())*(ig.system.width/2)+(ig.system.width*3.5);}
for(i=ig.system.width+this.currentPlatform.size.x;i<this.xSpace;i+=100){this.spawnEntity(Toast,i,this.yPos-100);}
this.currentPlatform=this.spawnObject();if(Math.random()<1/4&&this.currentPlatform.toString()!="ground4"){this.spawnEntity(PowerUp,this.xSpace+Math.random()*500,this.yPos-60);}
if(Math.random()<3/6&&this.currentPlatform.toString()=="ground4"){this.spawnEntity(Truck,this.xSpace+Math.random()*700+300,this.yPos-190)}
else if(this.currentPlatform.toString()=="ground4"){if(Math.random()<1/2){this.spawnEntity(Bird,this.xSpace+Math.random()+2650,this.yPos-120);}else{this.spawnEntity(Bird,this.xSpace+Math.random()+2650,this.yPos-30);}}}
break;case this.GAME_OVER:if(this.score>this.highscore){this.highscore=this.score;}
if(ig.input.state('space')){this.startGame();}
if(ig.input.state('back')){this.state=this.MENU;}
break;}
this.parent();},startGame:function(){this.state=this.GAME;this.score=0;this.spawnEntity(MovingSky,0,0);this.spawnEntity(MovingSky,ig.system.width,0);this.spawnEntity(MovingHills,0,0);this.spawnEntity(MovingHills,ig.system.width,0);this.spawnEntity(MovingDubois,ig.system.width,ig.system.height-300);this.spawnEntity(MovingGrass,0,0);this.spawnEntity(MovingGrass,ig.system.width,0);this.player=this.spawnEntity(Player,100,ig.system.height-200);this.initialPlatform=this.spawnEntity(Ground1,0,ig.system.height-80)
this.initialPlatform.isItAlive=true;this.yPos=Math.random()*80+ig.system.height-80-80;this.xSpace=(Math.random())*(ig.system.width/2)+(ig.system.width);for(i=0+this.initialPlatform.size.x;i<this.xSpace;i+=100){this.spawnEntity(Toast,i,this.yPos-100);}
this.currentPlatform=this.spawnEntity(Ground2,this.xSpace,this.yPos);},spawnObject:function(){var whichPlatform=Math.random();if(whichPlatform<(2/3)){return this.spawnEntity(Ground4,this.xSpace,this.yPos);}else if(whichPlatform<(7/9)){return this.spawnEntity(Ground1,this.xSpace,this.yPos);}else if(whichPlatform<(8/9)){return this.spawnEntity(Ground2,this.xSpace,this.yPos);}else{return this.spawnEntity(Ground3,this.xSpace,this.yPos);}},draw(){this.parent();switch(this.state){case this.MENU:this.background.draw(0,0);this.title.draw(150,50);this.transparentbox.draw(180,285);this.doggi.draw(550,200);this.font.draw("Press 'SPACE' to start!\nPress 'UP' for instructions!\nPress 'DOWN' for credits!\nHIGH-SCORE: "+this.highscore,ig.system.width/2,ig.system.height/2,ig.Font.ALIGN.CENTER);break;case this.INSTRUCTIONS:this.instructionsbackground.draw(0,0);this.font.draw("Good morning Doggi! You're going to be late to \nclass again?! Maneuver your way through UMass \nto get to COMPSCI 397J!\n\nINSTRUCTIONS:\nJump on the platforms, collect 'toast' to get a\nhigher score, and gather 'milk' to gain special\nabilities. Dodge pesky geese by jumping and sliding.\nSlip under foodtrucks by crawling! There is no time\nlimit, so try to beat that high score! Avoid getting\nblown off the sides of the screen!\n\nCONTROLS:\n'UP' (Hold) : Jump (Higher)\n'UP' + 'SPACE' (Hold) : Double Jump (Higher)\n'DOWN' : Crawl",50,50,ig.Font.ALIGN.LEFT);this.font.draw(">>Press SPACE to start!",450,550);break;case this.CREDITS:this.instructionsbackground.draw(0,0);this.font.draw("MUSIC:\n'Bossa Nova' - Rhythm Heaven Fever\n\nART:\nSky, Grass - Pokemon Ruby/Sapphire\nPlatforms - CaptainToog\nOther stuff - Me! :D Caroline Yu\n\nGAME ENGINE:\nImpactJS",50,50,ig.Font.ALIGN.LEFT);this.font.draw(">>Press SPACE to start!",450,550);break;case this.GAME:this.font.draw("SCORE: "+this.score+"\nHIGH-SCORE: "+this.highscore,0,0);break;case this.GAME_OVER:this.gameoverbackground.draw(0,0);this.gameoverwords.draw(250,150);this.font.draw("FINAL SCORE: "+this.score+"\nHIGH-SCORE: "+this.highscore+"\nPress 'SPACE' to restart\nPress 'BACKSPACE' for main menu",ig.system.width/2,ig.system.height/2-50,ig.Font.ALIGN.CENTER);break;}},});ig.main('#canvas',MyGame,60,800,600,1);});